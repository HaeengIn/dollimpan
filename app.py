...
def check_update():
    try:
        ...
        save_path =os.path.join(os.path.dirname(__file__), 'Dollimpan_new.exe')
        with req.get(download_url, stream=True) as r:
            r.raise_for_status()
        ...
        # 기존 파일 삭제 및 새 파일 이름 변경
        old_path = os.path.abspath(__file__)
        os.remove(old_path)
        os.rename(save_path, old_path)
        messagebox.showinfo('업데이트', '최신 버전으로 업데이트되었습니다. 프로그램을 다시 시작하세요.')
        root.quit()
